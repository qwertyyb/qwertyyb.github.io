...
![编译wechat_qrcode模块报错](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1gros8m5xf3j31a404mn00.jpg)
...
![替换掉错误的返回值](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1grousq5puzj322q0legtr.jpg)
...
![错误截图](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1grovmbosxpj31900d0gvw.jpg)

同样的文件，不同的问题，是因为生成的文件里面不包含命名空间 `std::` 前缀，所以我们也可以在对应的生成位置，简单粗暴的把这个错误的字符串替换掉，如下图:

![替换掉错误的参数值](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1grovpzmd45j321k0m2dno.jpg)
...
![修改tests.html](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1groyb8saxej312v0u0jxx.jpg)
...
![运行效果](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1grwoffk01sj30sm11ejyw.jpg)
...
![修改编译脚本](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1grwpqe9vmej321k0f4tez.jpg)

然后重新编译，编译完成后再次执行测试脚本，报错信息如下: 

![报错信息](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1grwpsnrtmzj31800fm41s.jpg)
...
![readNetFromCaffe](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1grwqgj648rj310404gaaz.jpg)
...
![删除cv::utils::fs::exists的调用](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1grwqm0in8yj31n70u0ws5.jpg)

然后重新编译，运行测试代码验证。运行效果如下图

![运行效果](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1groyy7b8iij31ea0sc10r.jpg)
...
![register_vector](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1grozb8qc20j317c0u0gt1.jpg)

看上去像是一个注册 `vector` 类型为对应JS类的方法，所以可以试试注册一个 `vector<std::string>` 的类型看是否OK。最终改动此文件如下: 

![注册StringVector类型](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1grozgh2y1fj31zo0je48c.jpg)

然后再重新执行编译命令进行编译，无报错。刷新页面，重新在控制台执行测试代码，结果如下: 

![执行结果](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1groznnndx8j30sy0ligq0.jpg)
...
![opencv_config.js.py](/assets/编译微信二维码引擎到webAssembly实践/008i3skNgy1grp1tj65wej31w00iun44.jpg)
